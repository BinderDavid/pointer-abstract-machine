...
data MachineState =
    MkMachineState MachineHead Stack (Heap ())

embedCommand :: Command -> MachineState
embedCommand (Cut tm cnt) = MkMachineState (CutState CBV tm (-1) cnt (-1)) emptyStack emptyHeap
...
-------------------------------------------------------------------------------
computeStep (MkMachineState hd@(CutState _ _ pt1 _ pt2) stack heap) | max pt1 pt2 + 1 < topOfStack stack = do
    let newStack = restrictStack stack (max pt1 pt2)
    pure (MkMachineState hd newStack heap, GarbageCollectionStep)
-------------------------------------------------------------------------------
-- Logical Steps
-------------------------------------------------------------------------------
-- Reducing a cut between a lambda abstraction and a call stack
computeStep (MkMachineState (CutState _ (TmLambda x funbody) _p1 (CntCallStack funarg cont) p2) stack heap) = do
    let newHead = CutState CBV funbody (topOfStack stack) cont p2
    let newStack = extendStack stack x (TermBinding funarg p2)
    let newState = MkMachineState newHead newStack heap
...
-- order.
computeStep (MkMachineState (CutState eo prd@(TmMu x (Cut l1 l2)) p1 cns@(CntMu y (Cut r1 r2)) p2) stack heap) = do
...
computeStep (MkMachineState (CutState _ (TmMu x (Cut tm cont)) _p1 cont' p2) stack heap) = do
...
computeStep (MkMachineState (CutState _ tm' p1 (CntMu y (Cut tm cnt)) _p2) stack heap) = do
...
computeStep (MkMachineState (CutState _ (TmVar x) p1 cnt p2) stack heap) = do
    (tm,p) <- lookupTermBinding p1 stack x
    pure (MkMachineState (CutState CBV tm p cnt p2) stack heap, TermVarStep)
-- Evaluating consumer variables
computeStep (MkMachineState (CutState _ tm p1 (CntVar x) p2) stack heap) = do
    (cnt,p) <- lookupContinuationBinding p2 stack x
    pure (MkMachineState (CutState CBV tm p1 cnt p) stack heap, ContinuationVarStep)
-------------------------------------------------------------------------------
-- Finish computation
-------------------------------------------------------------------------------
computeStep (MkMachineState (CutState _ _ _ CntTop _) _ _) =
